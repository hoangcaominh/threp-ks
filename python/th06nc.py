...
class Th06nc(KaitaiStruct):
    ...
    def _read(self):
        self.header = Th06nc.Header(self._io, self, self._root)
    ...
    class Header(KaitaiStruct):
        ...
        def _read(self):
            ...
            self.stage_offsets = []
            for i in range(7):
                self.stage_offsets.append(self._io.read_u8le())
    ...
    def stages(self):
        ...
        for i in range(7):
            ...
            if _on == 0:
                self._m_stages.append(Th06nc.Dummy(self._io, self, self._root))
            else:
                self._io.seek(self.header.stage_offsets[i])
                self._m_stages.append(Th06nc.Stage(self._io, self, self._root))
